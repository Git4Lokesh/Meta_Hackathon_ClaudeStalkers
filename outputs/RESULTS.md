...
| **v7-rewardfix (GeminiHugger, in flight)** | SFT + GRPO + **reward surgery**, rank 32 | 200 | _training_ | _training_ | **Primary candidate overall.** Same pipeline as v6-SFT-gemini plus `TIME_PRESSURE_PENALTY 0.01→0.005`, `PENALTY_CAP_FRACTION 0.40→0.10`, `FATAL_SCORE 0.01→0.001`. Designed to unstick task2/3/5/6 which sat flat at the 0.01 reward floor across all 800 episodes of v5 despite the model hitting 1-2 milestones per episode. **Live training snapshot at epoch 0.17 shows 0% at-floor rollouts** (vs v6 at epoch 0.67 showing 25% at-floor), confirming the reward-shaping hypothesis on the wire. If v7 beats v3's +0.046 composite delta on head-to-head eval, it becomes hero. |

...
| **v7-rewardfix (GeminiHugger, in flight)** | SFT + GRPO + **reward surgery**, rank 32 | 200 | _training_ | _training_ | **Primary candidate overall.** Same pipeline as v6-SFT-gemini plus `TIME_PRESSURE_PENALTY 0.01→0.005`, `PENALTY_CAP_FRACTION 0.40→0.10`, `FATAL_SCORE 0.01→0.001`. Designed to unstick task2/3/5/6 which sat flat at the 0.01 reward floor across all 800 episodes of v5 despite the model hitting 1-2 milestones per episode. **08:02Z snapshot (v7 epoch 0.31 vs v6 epoch 0.80):** v7 milestone mean 0.477 is still ahead of v6's 0.343, but v6 has climbed from its earlier 0.230 trough so the gap is narrower than the 07:40Z peak suggested (2.6× → 1.4×). Both runs are real candidates; the head-to-head eval will pick the winner. |

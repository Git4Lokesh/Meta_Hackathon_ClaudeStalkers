...
| **v6-SFT (GeminiHugger, COMPLETED)** | SFT + GRPO, fixed loader, rank 32, 9 tasks | 200 (full) | **+0.01** | +0.10 (task2 0.05→0.15) | **Completed 2026-04-26 at 14:33 IST.** 7200 episodes on L40S. Training-time means: task1 0.460→0.573 (+0.113), **task2 0.010→0.248 (+0.238, 25×)**, task6 0.010→0.061, example_custom 0.946→0.976. Still-stuck: task3 0.010, task5 0.010 (motivates v7 reward fix). **Head-to-head eval**: composite 0.28 vs base 0.27 = +0.01, losing to v3's +0.046. task1 regressed to 0.68 (from v3's 0.748) — training signal partially transferred but not cleanly. Adapter: [`GeminiHugger/war-room-grpo-adapter-v6-sft`](https://huggingface.co/GeminiHugger/war-room-grpo-adapter-v6-sft). |
| **v7-fast (brodie1of1, COMPLETED)** | SFT + GRPO + reward surgery, rank 32, 6 tasks, **50 episodes** | 50 (fast variant) | **−0.033** | +0.10 (task2 0.05→0.15) | **Completed 2026-04-26 at 15:30 IST.** 300 episodes on L40S, ~75 min training. Lakshminath's v7 reward-fix (4 constants in `grader.py`) + SFT warm-start + 6-task mix. **Training-time strongest of any run**: task1 0.807, task2 0.512, task3 0.051 (FIRST non-zero task3 signal), overall mean 0.563 — ~2× v6's 0.287. **Eval composite 0.247 vs base 0.280 = −0.033, losing to BOTH v3 (+0.046) and base.** Task1 regressed sharply (0.78→0.58) as the reward surgery made the model more aggressive. Interesting behavioral signal: trained task3 rollouts had 4-9 pushbacks vs base's 0, meaning the model learned to push back but didn't learn the specific phrasing the verifier accepts. Adapter: [`brodie1of1/war-room-grpo-adapter-v7-fast`](https://huggingface.co/brodie1of1/war-room-grpo-adapter-v7-fast). |
| v7-rewardfix (GeminiHugger, full run) | SFT + GRPO + reward surgery, rank 32, 9 tasks | 200 × 9 = 7,200 eps | _eval pending_ | **+0.553 training-time** | **Completed 2026-04-26 ~16:30 IST.** The strongest training-time signal in the series. Lakshminath's full v7 landed overall `team_reward` mean **0.460** — 60% above v6's 0.287 and 100% above v5's 0.229. Per-task training-time lifts: **task2 0.010 → 0.565 (56×)**, task4 0.120 → 0.481 (4×), task6 0.010 → 0.234 (23×), procedural_hard 0.350 → 0.761 (+0.411). task3 finally lifted off the floor at 0.012 (first nonzero ever seen in any task3 training pass). task5 0.101 (10× v5's 0.010). Head-to-head eval deferred until after submission — v7-fast already demonstrated that training-time gains this aggressive partially transfer (task2 training 0.512 → eval 0.15) so we document this as strongest-training-signal rather than updating the documented eval hero. Adapter: [`GeminiHugger/war-room-grpo-adapter-v7-rewardfix`](https://huggingface.co/GeminiHugger/war-room-grpo-adapter-v7-rewardfix). Full metrics: `outputs/v7trainingresults/`. |

...
| **v6-SFT (GeminiHugger, COMPLETED)** | SFT + GRPO, fixed loader, rank 32, 9 tasks | 200 (full) | _eval in flight_ | _eval in flight_ | **Completed 2026-04-26 at 14:33 IST.** 7200 episodes on L40S. Training-time means: task1 0.460→0.573 (+0.113), **task2 0.010→0.248 (+0.238, 25×)**, task6 0.010→0.061, example_custom 0.946→0.976. Still-stuck: task3 0.010, task5 0.010 (motivates v7 reward fix). Adapter: [`GeminiHugger/war-room-grpo-adapter-v6-sft`](https://huggingface.co/GeminiHugger/war-room-grpo-adapter-v6-sft). Full analysis at `outputs/war_room_grpo_v6_sft/RESULTS.md`. Head-to-head eval running now (job `69edd4a6d2c8bd8662bcfa96` on brodie1of1, ~25 min). |
| **v7-rewardfix (GeminiHugger, in flight)** | SFT + GRPO + **reward surgery**, rank 32 | 200 | _training_ | _training_ | **Lakshminath's primary v7 run.** Same pipeline as v6-SFT plus `TIME_PRESSURE_PENALTY 0.01→0.005`, `PENALTY_CAP_FRACTION 0.40→0.10`, `FATAL_SCORE 0.01→0.001`. Designed to unstick task3/task5 which sat flat at the 0.01 reward floor in v5 and remained stuck in v6-SFT (confirming the reward-fix hypothesis is needed). **08:54Z snapshot at v7 epoch 0.51:** milestone mean **0.447** vs v6's final lifetime **0.308** (1.45× at half the training depth). v7 partial-credit fraction 86%, v6 23.5% at-floor. Still on track to finish after the 5pm submission deadline. |
| **v7-fast (brodie1of1, in flight)** | SFT + GRPO + reward surgery, rank 32, **6 tasks, 50 episodes** | 50 (fast, 6-task) | _training_ | _training_ | **Our fast variant to fit the 2-hour pre-submission window.** Same v7 reward patch as Lakshminath's, same SFT warm-start (`GeminiHugger/war-room-sft-v1-r32`). Trimmed to 50 eps × 6 tasks (task1/2/3/example_custom/procedural_easy/hard, dropped task4/5/6 which aren't in the scripted eval anyway). Target adapter: `brodie1of1/war-room-grpo-adapter-v7-fast`. Job `69edd303d70108f37acdff0e` on L40S. Expected: ~1h15m training + 25m eval = ~1h40m total, ~$2.50. |

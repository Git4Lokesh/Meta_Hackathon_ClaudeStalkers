...
| v7-rewardfix (fast variant) | 4 constants changed in `grader.py`, 50 eps × 6 tasks. | **−0.033** | Training-time strongest of any run (mean 0.594, task 2 0.512, first non-zero task 3 signal 0.051). But eval regressed: task 1 dropped from 0.748 to 0.58. |
| **v7-rewardfix (full run)** | Same constants, 200 eps × 9 tasks, completed ~16:30 IST on submission day. | _eval deferred_ | **Strongest training-time signal of the series.** Overall mean **0.460** (+60% vs v6, +100% vs v5). task 2 lifted 0.010 → **0.565 (56×)**, task 4 0.120 → 0.481, task 6 0.010 → 0.234, procedural_hard → 0.761. First-ever non-zero task 3 (0.012). Eval queued for post-submission. |
...
The last run to finish before submission was Lakshminath's full v7 — 200 episodes × 9 tasks, completed around 16:30 IST. It produced our strongest training-time signal of the entire series: overall `team_reward` mean **0.460** (+60% vs v6, +100% vs v5), task 2 lifted from 0.010 to **0.565 (56×)**, task 4 from 0.120 to 0.481, procedural_hard to 0.761, and we saw the first-ever non-zero task 3 signal (0.012). Head-to-head eval is queued post-submission. Given what v7-fast taught us, we expect eval transfer to be real but partial — the reward-fix theory is doing what it's supposed to do at training time.
